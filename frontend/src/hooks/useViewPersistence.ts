...
export type ExplorerViewMode = "list" | "tree";
 ...
 *
 * Supports Explorer view modes: list and tree.
 *
 * Stores view preferences in localStorage with the following keys:
 * - `orbit:view:${folderPath}` - The view mode (list, tree)
...
export function useViewPersistence(
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (storedView) {
        const validModes: ExplorerViewMode[] = ["list", "tree"];
