    ...
    for color in colors {
        ...
        for existing in &mut merged_colors {
            if color_distance(&color, existing) < 30.0 {
                ...
                existing.r = ((existing.r as f32 * existing.percentage + color.r as f32 * color.percentage) / total) as u8;
                existing.g = ((existing.g as f32 * existing.percentage + color.g as f32 * color.percentage) / total) as u8;
                existing.b = ((existing.b as f32 * existing.percentage + color.b as f32 * color.percentage) / total) as u8;
